...
if __name__ == '__main__':
    ...
    lr.regress([log(each) for each in pDoc.rlonx],
        [log(each) for each in pDoc.rlony], npts2)
    ...
    r1 = (pDoc.rlonx, [exp(log(each) * x[1] + x[0])
        for each in pDoc.rlonx], x[2])
    temp = [exp(log(each) * x[1] + x[0]) for each in pDoc.rlonx]
    residuals = [(pDoc.rlony[each]) - (temp[each])
    ...
    '''lr.regress([log(each) for each in pDoc.rlonx],
            [log(each) for each in pDoc.rlony],pDoc.npts)
    x = [lr.intercept,lr.slope,lr.rms,0.0]
    ...
    print('temp: '+str(temp))
    residuals = [(pDoc.rlony[each])
        - (temp[each]) for each in range(len(temp))]
    #print(r1)
