...
Read in Data
```{r}
folderLocation = 'P:/RXCA/Colin Gohmann/signalResponseRCode'
#folderLocation = 'P:/RXCA/Christie_Henry/POD/SignalResponseExample'
data = read.csv(paste(folderLocation,'/dataFromPlots.csv',sep=""), header=FALSE, col.names=c("y","x"))
summary(data)
```

```{r}

data_obs=na.omit(data)
...
Fitting a linear model (primarily for plotting purposes AND for simulating more data).
```{r}
...
# Save out the file
write.table(new_data,"Plot_Data_50.csv")

data_obs = new_data[c("x","y")]

```

Transforming x (using identity here).
```{r}
# Function transforming the flaw size. Uncomment 1
f_a=function(a){a #log(a) #exp(a) #a^2
  }
# Inverse function of f_a. Uncomment 1
f_a_i=function(a){ a # exp(a)#log(a) #sqrt(a)
  }
...
Box Cox Y transformation (setting lambda to 1, so no transformation)
```{r}
## Use boxcox function to find the best transformation 
#bc<-boxcox(y ~ x.trans, data=data_obs, na.action=na.omit, lambda = seq(-2,2,by=0.01),
#                    plotit = TRUE, eps = .Machine$double.eps/2, 
#                    xlab = expression(lambda), ylab = "LL")
#lambda <- bc$x[which.max(bc$y)]
#print(paste("Box Cox Lambda is ",lambda))
## If lambda not zero, then (y^lambda-1)/lambda, else use log(y).
#lambda=1
#  if(lambda!=0){
#    data_obs$y.trans = (data_obs$y^lambda-1)/lambda
#  }else{
#    data_obs$y.trans = log(data_obs$y)
#  }
...
```
Refitting the model with the 50 points.
```{r}
# Fitting a linear model
...
Check out this site for the interpretations of the residual plots.
...
shapiro.test(studres(linearModel_lm))
...
durbinWatsonTest(linearModel_lm)
```


Fitting a survival model; lm does not return all the varcov terms needed for the transform to probability. This is the same model as lm. 
...
```{r}
tibble(
  Metric=c("a.50", "a.90", "a.90.95"),
  DefectSize=c(a.50, a.90, a.90.95)
)
```
