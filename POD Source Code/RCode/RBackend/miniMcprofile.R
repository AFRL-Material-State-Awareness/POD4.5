...
#######################################################################################################
# ####This function is used for MLR only
...
#   j.1 <- j * vc
#   newFastList<<-as.array(unlist(object$cobject, recursive=FALSE))
#   # currRow=c()
#   # j.0=c()
#   # ptm <- proc.time()
#   # for(i in 1:length(newFastList)){
#   #   print(i)
#   #   xt <- try(1/(detvarorglm(newFastList[[i]])), silent = TRUE)
#   #   if (class(xt)[1] == "try-error") xt <- NA
#   #   xt[xt > 10^12] <- NA
#   #   currRow=append(currRow, xt)
#   #   if(i%%length(object$cobject[[1]])){
#   #     j.0=append(j.0, list(currRow))
#   #     currRow=c()
#   #   }
#   # }
#   ptm <- proc.time()
#   clust=makeCluster(detectCores())
#   clusterExport(clust, list("newFastList", "detvarorglm"))
#   j.0<-parSapply(clust, 1:length(newFastList), function(i){
#     xt <- try(1/(detvarorglm(newFastList[[i]])), silent = TRUE)
#     if (class(xt)[1] == "try-error") xt <- NA
#     xt[xt > 10^12] <- NA
#     #currRow=append(currRow, xt)
