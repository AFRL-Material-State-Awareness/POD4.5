...
class linear_regression():
    ...
    def regress(self, x, y, npts):
        ...
        valResult = cyy-slope*cxy
        if valResult > 0.0:
            self.rmse=sqrt(valResult);
        else:
            self.rmse = 0.0
        SSres = ssy - b1 * ssx
        ...
        valResult = self.anova[7]*sumx2/((npts*sumx2)-(sumx*sumx))
        if valResult > 0.0:
            self.stat[5]=sqrt(valResult)
        else:
            self.stat[5] = 0.0

        valResult = self.anova[7]*npts/((npts*sumx2)-(sumx*sumx))
        if valResult > 0.0:
            self.stat[1]=sqrt(valResult)
        else:
            self.stat[1] = 0.0
